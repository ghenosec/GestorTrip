...
app.whenReady().then(() => {
  ...
  ipcMain.handle("session:save", (_, user) => {
    try {
      fs.writeFileSync(sessionPath, JSON.stringify(user))
    } catch {}
  })
  ...
  ipcMain.handle("db:export", async () => {
    const { canceled, filePath: destPath } =
      await dialog.showSaveDialog(mainWindow, {
        title: "Exportar banco de dados",
        defaultPath: `gestortrip-backup-${new Date()
          .toISOString()
          .slice(0, 10)}.db`,
        filters: [
  ...
  ipcMain.handle("relatorio:gerar", async (_, htmlContent) => {
    const { canceled, filePath: destPath } =
      await dialog.showSaveDialog(mainWindow, {
        ...
        defaultPath: `relatorio-gestortrip-${new Date()
          .toISOString()
          .slice(0, 10)}.pdf`,
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        win.webContents.once("did-fail-load", (_, errCode, errDesc) =>
          reject(
            new Error(
              `Falha ao carregar HTML: ${errDesc} (${errCode})`
            )
          )
        )
